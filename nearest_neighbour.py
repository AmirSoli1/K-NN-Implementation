...
def predictknn(classifier, x_test: np.array):
    ...
    for test_example in x_test:
        # Calculate Euclidean distances between test example and all training examples
        distances = np.linalg.norm(x_train - test_example, axis=1)

        # Get indexes of k nearest neighbors
        nearest_neighbors_indexes = np.argsort(distances)[:k]

        # Get labels of k nearest neighbors
        nearest_neighbors_labels = y_train[nearest_neighbors_indexes].astype(int)
